...
The Ellipse Calculator generates an ellipse given major and minor axes lengths and a rotation angle in degrees. The parameters may be constants or come from an incoming GeoEvent

![Image of Ellipse Processor](ellipse.png)
...
* Open a command prompt and navigate to `solutions-geoevent-java/solutions-geoevent/processors/ellipse-processor`
    * Enter `mvn install` at the prompt.

## Installation

* Install the Ellipse Processor.
...
You will use the Ellipse-Test service from the Solutions Test Configuration to test the functionality of the ellipse calculator.
* Open the Ellipse-Test service in GEE Manager. 
* Click on the ellipse-tcp-txt-in Input and expand the 'Advanced' tab. Note that the input uses TCP port 5601.
...
* Select ellipse.csv and click 'Open'
...
* In the TCP-console you will see that the Received Event (Event Definition name) is ellipse-in with a comma separated list of values. The last attribute value will be a json representation of the ellipse generated by the processor.
