...
Returns the Center of mass of an incoming event's polygon geometry  

![Image of P2P](p2p_390X150.png)
...
* Open a command prompt and navigate to `solutions-geoevent-java/solutions-geoevent/10.3.0/processors/reducePolygonToPoint-processor`
    * Enter `mvn install` at the prompt.

## Installation

* Install the Extract Point from Polygon.
    * Browse to `solutions-geoevent-java/solutions-geoevent/processors/10.3.0/reducePolygonToPoint-processor/target` (this directory is created when you execute mvn install).
...
* See the [solutions-geoevent-java validation instructions](../../../README.md#validating-install).
    * Ensure the Extract Polygon from Point Processor exists.
...
You will use the Polygon-to-Point-Test service from the Solutions Test Configuration to test the functionality of the extract point from polygon processor. This service first generates a buffer and sets the geometry to the buffer geometry. Next it uses the Extract Polygon From Point processor to convert the geometry back into a point.
* Open the Polygon-to-Point-Test service in GEE Manager. 
* Click on the polygon2pt-tcp-txt-in Input and expand the 'Advanced' tab. Note that the input uses TCP port 5613.
...
* In the upper right, change the port to 5613 and  click the button with the red X to connect (Note: if you cannot connect the server is not listening on that port. This may be because the Input in GeoEvent extension has not been started).
...
* Select addxyx.csv and click 'Open' (Note: the addxyz.csv simulation file is used for multiple tests).
...
* In the TCP-console you will see that the Received Event (Event Definition name) is addxyz-in with a comma separated list of values. The geometry will be a json point.
